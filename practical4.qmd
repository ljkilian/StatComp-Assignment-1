...
::: {.callout-warning}
## Note on Truncation
Most tables have been truncated using the head() function in the interest of readability.
:::
...
# Question 3
## Question 3.1
We can see here that there is only one observation on carrier "OO" in month 1, thus when calculating our standard deviation in the previous table we receive an NA.
```{r}
#| echo: true
#| message: false
check1 <- 
...
## Question 3.2
Here we see that in month 1, carriers F9, YV, AS and HA flew identical distances every time, thus leading to us having a standard deviation of 0 
```{r}
check2 <- 
  flights %>%
  filter(carrier == "F9") %>%
...
kable(head(worst_diff))
```

## Question 6.5
It seems that ExpressJet had only one terribly delayed flight (as seen in table 2) from JFK to ATL and thus skews the average to that singular data point, thus leading to it becoming the worst difference
```{r}
#| echo: true
#| message: false
reason <- 
...
# Question 7
Gender has a typo "femal" \n
Disease status has a duplicate "healthy" and "Healthy" \n
Glucose and height have NA entries \n
